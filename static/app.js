...
class SuperBizAgentApp {
    ...
    async sendAIOpsRequest(loadingMessageElement) {
        try {
            const response = await fetch(`${this.apiBaseUrl}/aiops/diagnose`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({
                    ...
                    question: '请诊断 Kubernetes 集群中 checkout-api 的健康状态，并给出可审批处置建议',
                    include_actions: true,
                    alert: {
                        name: 'KubernetesHealthCheck',
                        severity: 'warning',
                        summary: '用户从 Web 触发 Kubernetes AIOps 诊断',
                                    ...
                                    for (const jsonStr of matches) {
                                        try {
                                            ...
                                            if (sseMessage.type === 'report') {
                                                ...
                                                fullResponse += this.formatAIOpsEvent(sseMessage);
                                            }
                                        } catch (e) {
    ...
    formatAIOpsEvent(event) {
        ...
        if (event.type === 'evidence') {
            if (event.evidence) {
                return `\n- 证据 ${event.evidence.evidence_id}: ${event.evidence.summary || event.message || ''}`;
            }
            return `\n${event.message || ''}\n`;
        }
        if (event.type === 'hypothesis') {
            const hypotheses = event.hypotheses || [];
            const items = hypotheses.map(item => `- ${item.title} (${Math.round((item.confidence || 0) * 100)}%)`).join('\n');
            return `\n\n## 🔍 根因假设\n${items}\n`;
        }
        if (event.type === 'action_plan') {
            const actions = event.actions || [];
            const items = actions.map(item => `- ${item.title}，风险: ${item.risk_level}，需审批: ${item.requires_human_approval}`).join('\n');
            return `\n\n## 🛠️ 可审批处置建议\n${items}\n`;
        }
