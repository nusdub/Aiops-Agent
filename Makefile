# Kubernetes AIOps Agent Makefile
# 用于自动化项目初始化、Docker 管理和文档向量化

# ============================================================
# 配置变量
# ============================================================
PYTHON ?= $(shell if [ -x .venv/bin/python ]; then echo .venv/bin/python; elif command -v python3 >/dev/null 2>&1; then echo python3; elif command -v python >/dev/null 2>&1; then echo python; else echo python3; fi)
HOST ?= 0.0.0.0
PORT ?= 9900
...
DOCS_DIR = aiops-docs
K8S_MCP_CONTAINER = kubernetes-aiops-mcp
...
.PHONY: help init start stop restart check upload clean wait \
        install install-dev dev run test test-quick format lint fix type-check \
        compile security pre-commit-install pre-commit check-all coverage docs shell \
        ipython watch add add-dev remove list-docs test-upload sync logs \
        docker-build docker-up docker-down docker-restart docker-logs docker-status docker-test
...
help:
	@echo "$(GREEN)═══════════════════════════════════════════════════════$(NC)"
	@echo "$(GREEN)  Kubernetes AIOps Agent - Makefile 命令$(NC)"
	@echo "$(GREEN)═══════════════════════════════════════════════════════$(NC)"
	@echo ""
	@echo "$(CYAN)【一键操作】$(NC)"
	@echo "  $(YELLOW)make init$(NC)         - 🚀 一键初始化（依赖 → 服务 → 上传文档）"
	@echo ""
	@echo "$(CYAN)【Docker 管理】$(NC)"
	@echo "  $(YELLOW)make kind-demo$(NC)    - ☸️ 创建 kind 演示集群"
	@echo "  $(YELLOW)make docker-up$(NC)    - 🐳 启动完整 Linux 容器栈（App + Kubernetes MCP）"
	@echo "  $(YELLOW)make docker-down$(NC)  - 🛑 停止完整容器栈"
	@echo "  $(YELLOW)make docker-logs$(NC)  - 📜 查看完整容器栈日志"
	@echo ""
	@echo "$(CYAN)【服务管理】$(NC)"
	@echo "  $(YELLOW)make start$(NC)        - 🚀 启动 FastAPI 服务"
	@echo "  $(YELLOW)make stop$(NC)         - 🛑 停止 FastAPI 服务"
	@echo "  $(YELLOW)make restart$(NC)      - 🔄 重启服务"
	@echo "  $(YELLOW)make check$(NC)        - 🔍 检查服务状态"
	@echo ""
	@echo "$(CYAN)【开发模式】$(NC)"
	...
	@echo "  $(YELLOW)make compile$(NC)      - 🧱 Python 编译检查"
	@echo "  $(YELLOW)make fix$(NC)          - 🔧 自动修复问题"
	@echo "  $(YELLOW)make test$(NC)         - 🧪 运行测试"
	@echo "  $(YELLOW)make check-all$(NC)    - ✅ 运行非改写型质量门禁"
	@echo ""
	@echo "$(CYAN)【其他】$(NC)"
	...
	@echo "$(GREEN)═══════════════════════════════════════════════════════$(NC)"
	@echo "$(GREEN)使用示例:$(NC)"
	@echo "  1. Linux 生产部署: $(YELLOW)make docker-up$(NC)"
	@echo "  2. 本地开发:       $(YELLOW)make init$(NC)"
...
init:
	@echo "$(GREEN)═══════════════════════════════════════════════════════$(NC)"
	@echo "$(GREEN)🚀 开始一键初始化 Kubernetes AIOps Agent...$(NC)"
	@echo "$(GREEN)═══════════════════════════════════════════════════════$(NC)"
	@echo ""
	@echo "$(YELLOW)步骤 1/3: 安装 Python 依赖$(NC)"
	@$(MAKE) install
	@echo ""
	@echo "$(YELLOW)步骤 2/3: 启动服务$(NC)"
	@$(MAKE) start-api
	@echo ""
	@echo "$(YELLOW)步骤 3/3: 等待服务就绪并上传文档$(NC)"
	@$(MAKE) wait
	@$(MAKE) upload
	@echo ""
	@echo "$(GREEN)═══════════════════════════════════════════════════════$(NC)"
	@echo "$(GREEN)✅ 初始化完成！知识库文档已向量化存储到 Chroma$(NC)"
	@echo "$(GREEN)═══════════════════════════════════════════════════════$(NC)"
	@echo ""
	@echo "$(GREEN)🌐 服务访问地址:$(NC)"
	@echo "   Web 界面: $(SERVER_URL)"
	@echo "   API 文档: $(SERVER_URL)/docs"
	@echo "   Kubernetes MCP: http://localhost:8010/mcp"
	@echo ""
...
# 构建完整 Linux 容器镜像
docker-build:
	@echo "$(YELLOW)🐳 构建 Kubernetes AIOps Agent 容器镜像...$(NC)"
	$(DOCKER_COMPOSE) -f $(COMPOSE_FILE) build
	@echo "$(GREEN)✅ 容器镜像构建完成$(NC)"

# 启动完整 Linux 容器栈（App + Kubernetes MCP）
docker-up:
	@echo "$(YELLOW)🐳 启动完整 Linux 容器栈...$(NC)"
	$(DOCKER_COMPOSE) -f $(COMPOSE_FILE) up -d --build
	@echo "$(GREEN)✅ 容器栈启动命令已执行$(NC)"
	@echo "$(YELLOW)   Web: http://localhost:$(PORT)$(NC)"
	@echo "$(YELLOW)   Docs: http://localhost:$(PORT)/docs$(NC)"

# 停止完整 Linux 容器栈
docker-down:
	@echo "$(YELLOW)🛑 停止完整 Linux 容器栈...$(NC)"
	$(DOCKER_COMPOSE) -f $(COMPOSE_FILE) down
	@echo "$(GREEN)✅ 容器栈已停止$(NC)"
...
# 执行 Linux 容器部署测试脚本
docker-test:
	bash scripts/test_linux_deploy.sh
...
start:
	...
	@echo "$(GREEN)═══════════════════════════════════════════════════════$(NC)"
	@echo ""
	@echo "$(YELLOW)💡 本地脚本仅启动 FastAPI；完整 Linux 栈请使用 make docker-up$(NC)"
	@echo ""
	@$(MAKE) start-api
	@echo ""
	@echo "$(GREEN)═══════════════════════════════════════════════════════$(NC)"
...
start-api:
	...
	@if curl -s -f $(HEALTH_CHECK_API) > /dev/null 2>&1; then \
		...
		nohup $(PYTHON) -m uvicorn app.main:app --host $(HOST) --port $(PORT) > server.log 2>&1 & \
		echo $$! > server.pid; \
...
check:
	...
	@if curl -s -f $(HEALTH_CHECK_API) > /dev/null 2>&1; then \
		...
		echo ""; \
		echo "$(CYAN)健康检查响应:$(NC)"; \
		curl -s $(HEALTH_CHECK_API) | $(PYTHON) -c "import sys,json; print(json.dumps(json.load(sys.stdin), indent=2, ensure_ascii=False))" 2>/dev/null || curl -s $(HEALTH_CHECK_API); \
	else \
...
lint:  ## 代码检查
	@echo "$(YELLOW)🔍 代码检查...$(NC)"
	$(PYTHON) -m ruff check app tests mcp_servers
	@echo "$(GREEN)✅ 检查完成$(NC)"
...
compile:  ## Python 编译检查
	@echo "$(YELLOW)🧱 Python 编译检查...$(NC)"
	$(PYTHON) -m compileall -q app mcp_servers tests
	@echo "$(GREEN)✅ Python 编译检查完成$(NC)"
