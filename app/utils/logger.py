...
def setup_logger():
    ...
    enqueue_logs = config.log_enqueue if config.log_enqueue is not None else os_name != "nt"
    ...
    # 添加文件输出（按天轮转，自动压缩）
    logger.add(
        str(config.log_path / "app_{time:YYYY-MM-DD}.log"),
        rotation="00:00",  # 每天0点自动切割新日志文件
        retention="7 days",  # 仅保留最近7天的日志
        compression="zip",  # 过期日志自动压缩为zip
        encoding="utf-8",  # 解决中文乱码
        enqueue=enqueue_logs,  # Linux 默认异步写入，受限环境可通过 LOG_ENQUEUE=false 关闭
        backtrace=True,  # 显示完整异常栈信息
